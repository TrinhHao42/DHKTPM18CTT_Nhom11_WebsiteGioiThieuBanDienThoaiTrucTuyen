...
import { getAllReturnRequests, getReturnRequestDetail, processReturnRequest, ReturnRequestItem, ReturnRequestDetail } from '@/services/requestService';
...
export default function ReturnRequestTable() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<ReturnRequestDetail | null>(null);
  ...
  const fetchRequests = async () => {
    try {
      setLoading(true);
      const data = await getAllReturnRequests(currentPage, 10, statusFilter);
  ...
  const handleViewDetail = async (id: number) => {
    try {
      const detail = await getReturnRequestDetail(id);
  ...
  const handleProcess = async (action: 'APPROVE' | 'REJECT', adminNote?: string) => {
    ...
    try {
      setProcessing(true);
      await processReturnRequest(selectedRequest.returnRequestId, action, adminNote);
              ...
              {filteredRequests.map((request) => (
                <TableRow key={request.returnRequestId} className="transition-colors hover:bg-gray-50 dark:hover:bg-white/[0.02]">
                  ...
                  <TableCell className="py-3 text-center">
                    <button
                      onClick={() => handleViewDetail(request.returnRequestId)}
    ...
    <div className="fixed inset-0 z-[999999] flex items-center justify-center bg-black/50 p-4">
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white shadow-2xl dark:bg-gray-900">
        ...
        <div className="space-y-6 p-6">
          ...
          <div>
            <h4 className="mb-3 font-semibold text-gray-900 dark:text-white">Lý do trả hàng</h4>
            <div className="rounded-lg bg-gray-50 p-4 dark:bg-gray-800">
